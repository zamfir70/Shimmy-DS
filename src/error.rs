...
mod tests {
    ...
    fn test_model_load_error() {
        ...
        assert_eq!(error.to_string(), "Model loading failed: /path/to/model.bin");
    ...
    fn test_config_error() {
        ...
        assert_eq!(error.to_string(), "Invalid configuration: temperature = invalid");
    ...
    fn test_template_error() {
        ...
        let source_error: Box<dyn StdError + Send + Sync> = 
            Box::new(std::io::Error::new(ErrorKind::InvalidData, "Invalid template syntax"));
        ...
        assert_eq!(error.to_string(), "Template rendering failed: chat_template");
        assert!(format!("{:?}", error).contains("TemplateError"));
        assert!(format!("{:?}", error).contains("chat_template"));
        
        // Test source error chaining
        assert!(error.source().is_some());
        assert!(error.source().unwrap().to_string().contains("Invalid template syntax"));
    }
    ...
    fn test_io_error_conversion() {
        ...
        // Test source error chaining
        assert!(error.source().is_some());
        assert!(error.source().unwrap().to_string().contains("File not found"));
    }
    ...
    fn test_error_debug_formatting() {
        let errors = vec![
            ShimmyError::ModelNotFound { name: "test".to_string() },
            ShimmyError::GenerationError { reason: "failed".to_string() },
            ShimmyError::ConfigError { 
                field: "field".to_string(), 
                value: "value".to_string() 
            },
            ShimmyError::BackendNotAvailable { backend: "backend".to_string() },
        ];
    ...
    fn test_error_display_formatting() {
        let test_cases = vec![
            (
                ShimmyError::ModelNotFound { name: "llama".to_string() },
                "Model not found: llama"
            ),
            (
                ShimmyError::GenerationError { reason: "timeout".to_string() },
            ...
            (
                ShimmyError::BackendNotAvailable { backend: "openai".to_string() },
    ...
    fn test_error_source_chain() {
        ...
        assert_eq!(model_error.source().unwrap().to_string(), "Root cause error");
        ...
        assert!(template_error.source().unwrap().to_string().contains("Template file not found"));
    }
    ...
    fn test_error_variants_exhaustiveness() {
        ...
        let errors = vec![
            ...
            ShimmyError::GenerationError { reason: "test".to_string() },
            ShimmyError::ConfigError { 
                field: "test".to_string(),
                value: "test".to_string(),
            },
            ShimmyError::BackendNotAvailable { backend: "test".to_string() },
