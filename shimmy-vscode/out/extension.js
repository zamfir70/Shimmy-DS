...
async function startServer() {
    ...
    const config = vscode.workspace.getConfiguration('shimmy');
    const binaryPath = config.get('binaryPath') || 'shimmy';
...
async function checkServerStatus() {
    const config = vscode.workspace.getConfiguration('shimmy');
    const serverUrl = config.get('serverUrl') || 'http://localhost:11435';
    try {
...
async function makeShimmyRequest(serverUrl, prompt) {
